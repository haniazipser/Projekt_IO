...
extension View {
    func overlayFallback(viewState: ViewState, fetchData: @escaping (_ forceRefresh: Bool) async -> Void) -> some View {
        overlay {
            switch viewState {
            case .loading:
                ProgressView()
...
struct CourseView: View {
    ...
    @State private var apiError: ApiError?
        ...
        var viewState: ViewState {
            if !networkMonitor.isConnected && !loaded {
    ...
    var body: some View {
        NavigationStack {
            ...
            .overlayFallback(viewState: viewState, fetchData: fetchData)
